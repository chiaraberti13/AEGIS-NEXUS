...
Sensors also send a signed heartbeat to `POST /api/v1/sensors/heartbeat` on a separate operational channel. Heartbeats are stored outside the attack-event/session tables, so a quiet but healthy sensor does not inflate SOC event counts. `AEGIS_SENSOR_HEARTBEAT_INTERVAL_SECONDS` defaults to 60 seconds and the collector marks contact `stale` after `AEGIS_SENSOR_HEARTBEAT_STALE_SECONDS` (default 180 seconds).
